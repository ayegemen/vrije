...
from .models import Article, City, Mensen, Standpunten, Punten
...
def city(request, city_name):
    ...
    mens = Mensen.objects.filter(city=city, published=True).order_by('title')[:1]
    mensen = Mensen.objects.filter(city=city, published=True).extra(select={'myinteger': "CAST(substring(title FROM '[0-9]+') AS INTEGER)"}).order_by('myinteger')[:8]
...
def contact(request, city_name):
    ...
    city = get_object_or_404(City, title=city_name)
    adres = city.bezoek_adres.splitlines()[1]
...
def mensen(request, city_name):
    ...
    city = get_object_or_404(City, title=city_name)
    try:
        #mensen = Mensen.objects.filter(city=city, published=True).order_by('title')
        #mensen = Mensen.objects.filter(city=city, published=True).extra(select={'myinteger': "CAST(substring(title FROM '^[0-9]+') AS INTEGER)"}).order_by('myinteger')
        mens = Mensen.objects.filter(city=city, published=True).order_by('title')[:1]
        mensen = Mensen.objects.filter(city=city, published=True).exclude(title='Lijsttrekker').extra(select={'myinteger': "CAST(substring(title FROM '[0-9]+') AS INTEGER)"}).order_by('myinteger')
        mensen = list(chain(mens, mensen))  
    except:
        raise Http404("contact the webmaster")   
...
def standpunten(request, city_name):
    ...
    city = get_object_or_404(City, title=city_name)
    standpunten = Standpunten.objects.filter(city=city).order_by('nummer')
...
def punten(request, city_name, punt_slug):
    ...
    city = get_object_or_404(City, title=city_name)
    punt = get_object_or_404(Punten, slug=punt_slug)
